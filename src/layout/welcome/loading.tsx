...
const Loading: FC = () => {
  ...
  function drawEL(ctx: any,c: number, angle = 0) {
    ...
    const cx: number = 250
    const cy: number = 150
    const x1: number = cx - (r + 30) * Math.cos(Math.PI / 180 * angle) * Math.PI / 300 * c
    const y1: number = cy + 20 - (r - 50) * Math.sin(Math.PI / 180 * angle) * Math.PI / 100 * c
    ...
    ctx.beginPath()
    if (c != 0) {
      ctx.arc(x1, y1, 8, Math.PI * 0, Math.PI * 2)
    } else {
      ctx.arc(x1, y1, 12, Math.PI * 0, Math.PI * 2)
      ctx.shadowBlur = 12
    }
    ctx.fill()
    ctx.closePath()
  ...
  function drawLine(ctx: any, el: any, angle = 0) {
    ...
    ctx.beginPath()
    ctx.ellipse(cx, cy + 20, r + 30, r - 50, 0, Math.PI * 0, Math.PI * 2, false)
    ctx.stroke()
    ctx.closePath()
    // 椭圆轨迹卫星绘制
    const x1: number = cx - (r + 30) * Math.cos(Math.PI / 180 * angle)
    const y1: number = cy + 20 - (r - 50) * Math.sin(Math.PI / 180 * angle)
    ...
    ctx.shadowBlur = 12
    ctx.beginPath()
    ctx.arc(x1, y1, 9, Math.PI * 0, Math.PI * 2)
    ctx.fill()
    ctx.closePath()
    // 正圆绘制
    ctx.lineWidth = 5
    ...
    ctx.shadowBlur = 8
    ctx.beginPath()
    ctx.arc(x, y, 8, Math.PI * 0, Math.PI * 2)
    ctx.fill()
    ctx.closePath()
